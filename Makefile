...
%.unfinished.epub: %.markdown $(ASSETS) $(ASSETS_EPUB)
	...
	pandoc \
		...
		--css=literature.css \
		--metadata='title:$(shell python jankyjson.py $(basename $<).json "titles-qs[0]")' \
		--metadata='author:$(shell python jankyjson.py $(basename $<).json "author-qs")' \
		-t epub3 \
		-o $@ \
		$<

%.epub: %.unfinished.epub
	ebook-convert $< $@ --embed-font-family=Kingsley --no-default-epub-cover
...
%.html: %.markdown $(ASSETS) $(ASSETS_HTML)
	pandoc \
		...
		--metadata='title:$(shell python jankyjson.py $(basename $<).json "titles-qs[0]")' \
		--metadata='pagetitle:$(shell python jankyjson.py $(basename $<).json "titles[0]")' \
		--metadata='author:$(shell python jankyjson.py $(basename $<).json "author-qs")' \
		--section-divs \
		--css=literature.css \
		--css=webpage.css \
